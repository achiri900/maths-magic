...
import { ArrowRight, RotateCcw, Trophy, Star, Home, CheckCircle, XCircle, Timer } from 'lucide-react';

interface MultiplyGameProps {
  onBack: () => void;
}
...
function generateQuestion(level: number): Question {
  const maxNum = level <= 2 ? 5 : level <= 4 ? 9 : 12;
...
export default function MultiplyGame({ onBack }: MultiplyGameProps) {
              ...
              <div className="bg-white rounded-3xl p-8 shadow-lg mb-6 text-center">
                <div className="flex items-center justify-center gap-4 text-5xl md:text-6xl font-black text-gray-800 mb-8">
                  <motion.span
                    ...
                    key={question.num1}
                    className="bg-blue-50 px-6 py-4 rounded-2xl"
                  >
                    {question.num1}
                  </motion.span>
                  ...
                  {question.options.map((option, index) => (
                    <motion.button
                      ...
                      className={`p-6 rounded-2xl text-2xl font-bold transition-all ${
                        selectedAnswer === null
                          ? 'bg-gradient-to-br from-blue-50 to-cyan-50 hover:from-blue-100 hover:to-cyan-100 text-gray-800 shadow-md hover:shadow-lg cursor-pointer'
